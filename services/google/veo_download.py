...
class VeoDownloader:
    ...
    def _collect_urls(self, obj: Any) -> List[str]:
        ...
        urls = []
        url_keys = {"gcsUrl", "gcsUri", "signedUrl", "signedUri", "downloadUrl", "downloadUri", "videoUrl", "url", "uri", "fileUri"}
        
        if isinstance(obj, dict):
            for key, value in obj.items():
                if key in url_keys and isinstance(value, str) and (value.startswith("http://") or value.startswith("https://") or value.startswith("gs://")):
                    urls.append(value)
                else:
    ...
    def poll_and_download(
        ...
        if pending:
            self.log(f"[Veo] Warning: {len(pending)} operations still pending after {max_polls} polls")
