...
class _KeyItem(QWidget):
    def __init__(self, kind:str, key:str):
        ...
        self.btn_test=QPushButton('Kiểm tra Kết nối'); self.btn_test.setObjectName('btn_check_kiem')
...
class KeyList(QWidget):
    def __init__(self, *, title:str, kind:str, initial=None):
        ...
        # PR#4: Increase list height to show 3 keys (120px)
        ...
        self.btn_add=QPushButton('Thêm Key'); self.btn_add.setObjectName('btn_primary_add')
        self.btn_add.clicked.connect(self._add_from_input)
        row.addWidget(self.ed_new); row.addWidget(self.btn_add); v.addLayout(row)
        actions=QHBoxLayout()
        self.btn_import=QPushButton('Nhập từ File (.txt)'); self.btn_import.setObjectName('btn_import_nhap')
        self.btn_test_all=QPushButton('Kiểm tra tất cả'); self.btn_test_all.setObjectName('btn_check_kiem_all')
